...
def generate_synthetic_query_llm_approach(
    ...
    prompt_tokens_length = (
        tokenizer.encode(prompt_without_document, return_tensors="pt")
        .to(device)
        .shape[1]
    )
    document_length = (
        tokenizer.encode(document, return_tensors="pt").to(device).shape[1]
    )
    ...
    if prompt_tokens_length + document_length + 100 >= 2048:
        ...
        encoded_input = tokenizer(
            document,
            max_length=2048 - prompt_tokens_length - 100,
            truncation=True,
            return_tensors="pt",
        )
        truncated_document = tokenizer.decode(
            encoded_input["input_ids"][0][: 2048 - prompt_tokens_length - 100]
        )
    ...
    for percentile in percentiles:
        ...
        if input_ids.shape[0] != 1 or input_ids.shape[1] >= 2048:
            ...
            print(
                "Length of problematic document: "
                + str(len(encoded_input["input_ids"][0]))
            )
            assert False
...
def generate_answer_llm_approach(
    ...
    question_length = (
        tokenizer.encode(question, return_tensors="pt").to(device).shape[1]
    )
    ...
    if prompt_tokens_length + document_length + question_length + 100 >= 2048:
        ...
        encoded_input = tokenizer(
            document,
            max_length=2048 - reduction_length,
            truncation=True,
            return_tensors="pt",
        )
        truncated_document = tokenizer.decode(
            encoded_input["input_ids"][0][: 2048 - reduction_length]
        )
        document = truncated_document.replace("</s>", "")
    ...
    if input_ids.shape[0] != 1 or input_ids.shape[1] >= 2048:
        print("Length of problematic input ids: " + str(input_ids.shape))
        print(
            "Length of problematic document: " + str(len(encoded_input["input_ids"][0]))
        )
        assert False
    outputs = model.generate(
        input_ids=input_ids,
...
def generate_contradictory_answer_examples(
    ...
    def remove_problematic_contradictory_phrases(text):
        ...
        problematic_phrases = [
            ...
            "The false information created is:",
            "Incorrect Answer: ",
        ]
    ...
    queries_dataset_copy = queries_dataset_copy.drop_duplicates(
    ...
    queries_dataset_copy = queries_dataset_copy.sample(
    ...
    for i in tqdm(range(len(queries_dataset_copy))):
        ...
        if few_shot_examples_for_contradictory_answers is None:
            contradictory_answer_generated = generate_contradictory_answer_from_context(
        ...
        contradictory_answer_generated = remove_problematic_contradictory_phrases(
            contradictory_answer_generated
        )
    ...
    queries_dataset_copy_2 = queries_dataset_copy_2.drop_duplicates(
        subset=["synthetic_query"]
    )
    ...
    total_answers = queries_dataset[queries_dataset["Answer_Relevance_Label"] == "Yes"][
        "generated_answer"
    ].tolist()
    ...
    queries_dataset = pd.concat(
        [queries_dataset, queries_dataset_copy, queries_dataset_copy_2],
        axis=0,
        ignore_index=True,
    )
