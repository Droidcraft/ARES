    ...
    for _ in range(5):
        ...
        if few_shot_examples is not None:
            for row in range(len(few_shot_examples)):
                user_prompt += (
                    "Question: " + few_shot_examples.iloc[row][query_id] + "\n"
                )
                user_prompt += (
                    "Document: " + few_shot_examples.iloc[row]["Document"] + "\n"
                )
...
def few_shot_context_relevance_scoring_local(
    system_prompt: str,
    query: str,
    document: str,
    model_choice,
    ...
    if not TOGETHER_API_KEY:
        sys.exit(
            "TOGETHER_API_KEY environment variable not set. Please set the variable."
        )
...
def few_shot_answer_faithfulness_scoring_local(
    system_prompt: str,
    query: str,
    document: str,
    answer: str,
...
def few_shot_answer_relevance_scoring_local(
    system_prompt: str,
    query: str,
    document: str,
    answer: str,
...
def few_shot_context_relevance_scoring_claude(
    ...
    if not ANTHROPIC_API_KEY:
        sys.exit(
            "ANTHROPIC_API_KEY environment variable not set. Please set the variable."
        )
...
def few_shot_answer_faithfulness_scoring_vllm(
    ...
    if not OPENAI_API_KEY:
        sys.exit(
            "OPENAI_API_KEY environment variable not set. Please set the variable."
        )
